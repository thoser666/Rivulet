...
docker run -d --name "$RECEIVER" --network "$NETWORK" "$IMAGE" sh -ceu 'gst-launch-1.0 -q -e ristsrc uri="rist://@:10080" ! fakesink sync=false' >/dev/null
sleep 2
docker run --rm --name "$SENDER" --network "$NETWORK" "$IMAGE" \
  ...
  ristsink uri="rist://$RECEIVER:10080" >/dev/null
