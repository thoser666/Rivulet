...
| Scene-item copy/paste API | Determinism and embeddability: scriptable scene composition, OBS 32.2 frontend-API parity | M6 | Engine-level duplicate/paste of scene items within and across scenes (deterministic offset placement, source reuse, undo integration), exposed through a scriptable API and covered by deterministic unit tests; no GUI-only coupling |
