...
| Source delete hotkey | Automation and cross-platform parity: OBS 32.2 added delete-as-hotkey on macOS; the action should exist on every platform | M5 | Delete-source action registered in the existing hotkey registry (alongside scene switch, mute, …), rebindable in the Hotkeys settings, with the platform matrix updated honestly and a deterministic action-dispatch test |
