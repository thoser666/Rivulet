...
| F-M3-003 | Medium | Multitrack Video | Per-track encoder/transport negotiation (beyond config count 1–4) is not wired. | Later protocol milestone (M9); multistreaming fan-out wiring tracked in issue [#70](https://github.com/thoser666/Rivulet/issues/70) | Two negotiated video tracks over a transport |
...
| F-M3-005 | Medium | VOD track | Dual-output recording side is wired (issue #78, Z78-1/2): an active `VodTrack` adds a third, independent `audio_src_vod` branch into the recording muxer (named `mux_rec` request pad, fed via `push_audio_vod`); the live FLV path is never touched. The routing UI (Settings selector) is shipped; the streaming FLV mux stage now carries the VOD-track marker (`metadatacreator=Rivulet-ivod` in the onMetaData — see the quality-gate design note for why it is not a literal `ivod` AMF key). | Closed (issue [#78](https://github.com/thoser666/Rivulet/issues/78)) — a Twitch-published `ivod` AMF contract would replace the marker with the native key | VOD audio lands in a separate track in the recording |
