...
| F-M3-005 | Medium | VOD track | Dual-output recording side is wired (issue #78, Z78-1/2): an active `VodTrack` adds a third, independent `audio_src_vod` branch into the recording muxer (named `mux_rec` request pad, fed via `push_audio_vod`); the live FLV path is never touched. Remaining: the routing UI (Settings selector) and streaming-side ivod mux-stage wiring. | M9 / streaming-UI follow-up (issue [#78](https://github.com/thoser666/Rivulet/issues/78)) | VOD audio lands in a separate track in the recording |
