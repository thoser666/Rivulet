...
| ui-005 | Medium | Motion | No `prefers-reduced-motion` handling: `theme::preview_fade_alpha` (`animate_bool`) and the scene-transition `request_repaint_after(16 ms)` loop animate regardless of the OS motion setting | `theme::MotionPreference` (System/Full/Reduced, persisted) + Settings selector; `theme::os_prefers_reduced_motion()` reads the OS setting (Windows SystemParametersInfoW, macOS `defaults read`, Linux gsettings; cached, re-probed every 10 s); reduced motion zeroes egui's `animation_time` (all `animate_bool` fades snap) and collapses scene-transition Fades to Cuts. Note: egui 0.36 has no `options().reduced_motion` yet — the preference is applied manually via `theme::apply_motion` |
...
| ui-007 | Low | Feedback | Errors surfaced only inline in the owning view (e.g. `scene_status`, `last_error`); a failure raised in a background tab stayed invisible until the user navigated there | `RivuletApp::collect_global_issue` aggregates the view-local status fields (typed by severity: Error > Warning > Info, ties to the earliest view); the sidebar footer shows the current issue with a severity prefix and expires after 30 s; clicking it jumps to the owning view. Inline displays stay untouched (additional mirror, not a replacement) |
| ui-008 | Low | Target size | Icon-only buttons (`⚙`, `🗑`) use egui's default `interact_size.y = 18`, which can undershoot the WCAG 2.2 minimum target size of 24×24 CSS px | `theme::MIN_TARGET_SIZE` (24 px) applied as the global `spacing.interact_size.y` floor in `theme::init` (both schemes — covers `Button`, `DragValue`, `Slider`, `Checkbox`, `ComboBox`, collapsing headers); new `theme::icon_button` pins an explicit 24×24 `min_size` for glyph-only controls (the glyph is smaller than any floor, and egui's `.small()` variant skips the floor entirely — the ⚙/🗑/↩/✕ buttons were migrated to it). Tests allocate real buttons via `ctx.run_ui` and assert the 24 px rects |
