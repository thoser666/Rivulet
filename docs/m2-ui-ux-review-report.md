...
| F-M2-002 | Medium | P3/P4 source rendering | Partially resolved: `SceneSnapshot` now composites real RGBA `SnapshotFrame`s per layer (crop/scale/rotation/opacity) via `attach_frames`/`render_rgba`; snapshot export feeds browser frames in directly. Remaining source kinds (image/webcam/game/screen/text/media/color) still fall back to deterministic layout tiles until their platform renderers are connected (#217). | Export a scene snapshot with native sources. | M9 renderer owner; documented follow-up | Retest when native renderer lands |
