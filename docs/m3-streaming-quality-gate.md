...
properties rather than the unsupported `uri` property. The smoke sender parses H.264, muxes it as MPEG-TS with packet alignment, and applies explicit RTP/MP2T caps before `ristsink`. The Ubuntu GStreamer RIST element does not accept a raw `mpegtsmux ! ristsink` link; explicit `application/x-rtp` caps make the media contract deterministic. CI now includes a dedicated RIST receiver smoke job using
...
Adaptive bitrate remains bounded by the existing policy and updates the active `video_enc` property when present. `service_streaming()` now samples `StreamHealthMonitor` and invokes the runtime controller in the pipeline-owner tick; cooldown and hysteresis prevent flapping. The monitor exposes rolling bitrate/FPS and dropped-frame ratio, which are the input contract for runtime decisions. Production hardware measurement remains gate evidence; CI validates only the deterministic report schema and budget limits.
...
The adaptive-bitrate implementation provides a bounded policy and applies changed values to the active `video_enc` bitrate property. `AdaptiveBitrateController` adds stable-sample and cooldown protection, while `StreamHealthMonitor` supplies rolling bitrate, FPS, sent/dropped-frame counters, and dropped ratio from the active pipeline. The resource-efficiency checker validates p95/p99 frame-time, 1% lows, CPU, memory, and optional GPU fields; real hardware baselines remain required before declaring the M3 performance gate complete.
