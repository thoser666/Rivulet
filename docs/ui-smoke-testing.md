...
Die Verträge werden in `tests/ui_smoke.rs` (`responsive_contract_keeps_controls_reachable_on_narrow_windows`), `tests/ui_accessibility.rs` (`narrow_layout_is_responsive`), `tests/ui_regression.rs` (alle Viewports inkl. 640×480), dem In-File-Test `stream_workspace_stays_reachable_on_narrow_windows_in_source` und dem ci_pinning-Guard `stream_workspace_controls_stay_reachable_on_narrow_windows` geprüft.
