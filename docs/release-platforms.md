...
2. The Rust build is fully offline: `packaging/flatpak/cargo/cargo-sources.json`
   (1239 crates from `Cargo.lock`, generated by the official
   `flatpak-cargo-generator` at the commit pinned in
   `packaging/flatpak/generate-cargo-sources.sh`) plus
   `packaging/flatpak/cargo/config.toml` (vendored-sources). Re-run the script
   whenever `Cargo.lock` changes; CI fails if the committed file drifts.
...
4. CI job `flatpak-build.yml` validates the manifest end to end on every push:
   ...
   SDK and the `rust-stable` extension, runs the offline build, the export and
   bundle, then the official Flathub lint (`org.flathub.flatpak-builder-lint`)
