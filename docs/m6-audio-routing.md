...
**Phase 5 (macOS system-loopback fallback for Application sources, 2026-09-14):** macOS has no per-application capture API, so the fallback is the honest one: the system loopback — the first input device matching a known loopback driver (BlackHole, Soundflower, VB-Cable; the same keyword list as the legacy System capture) — is captured **once** and delivered to *every* routed Application source, with per-source volume/filters/mute still applied independently by the engine. `rivulet-audio` gains a macos-gated `app_audio_macos` module with the same `AppAudioCapture`/`AppAudioProcess`/`list_audio_processes` surface as the other backends (48 kHz stereo f32 matching the routed appsrc caps; cpal typed streams with the mandatory explicit `play()`; convert + resample on the worker thread so the realtime callback only copies bytes; atomic-flag stop with clean joins). The picker lists the loopback device first (capturable) followed by regular inputs; a Mixer hint states the sharing semantics (`audio_app_fallback_hint`, EN/DE). Without a loopback driver installed, capture fails with the descriptive install hint instead of recording silence. GUI wiring is now `cfg(any(windows, linux, macos))` everywhere, pinned by the new `per_app_capture_gating_covers_all_backends` GUI test and the `m6_audio_routing_phase5_macos_fallback_is_pinned` ci_pinning guard. **Follow-up (Core Audio device capture — issue [#231](https://github.com/thoser666/Rivulet/issues/231), macOS slice, 2026-09-25):** the macOS leg of the device half, via cpal. Device ids follow the `core-audio-in:<name>` convention (the cpal-reported device name — the stable identifier cpal's public API exposes; Core Audio device UIDs are not reachable through cpal). `rivulet-audio` gains the macOS-gated `device_capture_macos` module: `list_audio_devices()` enumerates input devices with loopback-capable virtual drivers (BlackHole, Soundflower, Loopback, VB-Cable — the per-app keyword list) sorted first, and `AudioDeviceCapture` streams one input device as 48 kHz stereo f32, converting the device's native sample format/channels/rate on the worker thread (the `app_audio_macos` DSP pattern). Honest `OutputDevice` semantics: macOS has no render-loopback API, so an `OutputDevice` source is only capturable through a virtual loopback driver — the picker reports loopback drivers as output-capable and shows the localized install hint (`audio_source_output_device_loopback_hint`), and a capture for a non-loopback output selection fails with a descriptive error instead of silently recording silence. The GUI picker and capture lifecycle are un-gated to `cfg(any(windows, linux, macos))`; without a selection the legacy placeholders keep working. With this slice, #231's proposal list is complete: device capture now covers Windows, Linux and macOS.

 **Follow-up (PipeWire device capture — issue [#231](https://github.com/thoser666/Rivulet/issues/231), 2026-09-25):** the *Linux* mirror of the device half. The `DeviceTarget` carrier gains the `pw-src:<node id>` (source node — microphone, virtual device) and `pw-mon:<node id>` (sink monitor — what that output plays) conventions alongside the WASAPI ones, parsed by the same `DeviceTarget::parse` so the GUI wiring stays platform-agnostic. `rivulet-audio` gains the Linux-gated `device_capture_pw` module: `list_audio_devices()` drains the PipeWire registry for capturable nodes (`media.class = Audio/Sink` monitors first, then `Stream/Input/Audio` source nodes, session defaults first), resolving the default markers from the `default` metadata object (`default.audio.sink` / `default.audio.source` JSON payloads, best-effort — without wireplumber metadata the picker works, only unmarked). `AudioDeviceCapture` streams one node as 48 kHz stereo f32 through a capture stream with `target.object = <node id>` — monitor capture adds `stream.capture.sink = true` — from a private loop/context/connection per source (the per-app pattern, so teardown never disturbs other captures). The GUI device picker and the `routed_device_targets`/`sync_device_audio_captures` lifecycle are un-gated to `cfg(any(windows, linux))`: the Linux Mixer's Input/Output rows show the same picker (friendly `node.description` labels, live 2 s refresh, cleared selection when a node vanished), routed device sources get one capture thread each, and strips show the friendly node name. Without a selection the legacy placeholders keep working. Pinned by an extension of `wasapi_device_capture_surface_is_pinned` (reusing the shared i18n keys `audio_source_pick_device` …); no new i18n keys were needed.

 **Follow-up (WASAPI device capture — issue [#229](https://github.com/thoser666/Rivulet/issues/229), 2026-09-24):** the *device* half of the audio matrix. The Mixer's Input/Output kinds were hardcoded to the legacy `default_input`/`system_loopback` placeholders; on Windows they now offer a WASAPI endpoint picker instead. Device ids follow the `wasapi-out:<endpoint id>` / `wasapi-in:<endpoint id>` convention (parsed by the new `DeviceTarget` in `rivulet-core`, the stable endpoint string so persisted configs survive reboots). `rivulet-audio` gains a Windows-gated `device_capture` module: `list_audio_devices()` enumerates active render/capture endpoints with friendly names and the console-default marker (render first, default first), and `AudioDeviceCapture` streams one endpoint as 48 kHz stereo f32 — shared-mode render **loopback** for output endpoints (e.g. the SteelSeries GG/Sonar virtual mixer channels), plain shared-mode capture for input endpoints, `AUTOCONVERTPCM | SRC_DEFAULT_QUALITY` converting the endpoint's native mix format so no resampling lives here. The GUI device picker mirrors the process picker (cached list, manual refresh, pending-selection hint, localized default marker) and re-enumerates live while it is open — bounded to one `MMDevice` enumeration every 2 s (the game-window-picker pattern), so plugged/unplugged endpoints appear or drop out without a restart and a selection is cleared when its endpoint vanished; routed device sources get one capture thread each via the same mpsc-drain lifecycle as the per-app path, and strips show the friendly endpoint name instead of the raw device id. Without a selection the legacy placeholders keep working, and non-Windows platforms are unchanged (macOS/Linux parity is a follow-up). Pinned by the `wasapi_device_capture_surface_is_pinned` ci_pinning guard; i18n keys `audio_source_pick_device`, `audio_source_refresh_devices`, `audio_source_no_device_selected`, `audio_source_default_device`, `audio_source_device_required`, `audio_device_capture_failed` (EN/DE).
...
for the honest platform-evidence follow-ups). The #229 WASAPI device-capture
follow-up above ships with the same quality gates (unit + behavior tests,
i18n parity guard).
