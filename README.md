...
| M2 – Scenes & Composition | Scenes, Sources, Game Capture, Scene Organisation, Transitions, Studio Mode | 🚧 In progress (scene management + G2 DXGI done; G3 Vulkan infra + tests; G4–G6 hooks open; S1–S8 sources open) | — | [![M2](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.github.com%2Frepos%2Fthoser666%2FRivulet%2Fmilestones%2F1&query=open_issues&label=M2&color=blue)](https://api.github.com/repos/thoser666/Rivulet/milestones/1) |
...
- [ ] **G3 – Vulkan hook** — VK_LAYER-based capture (OBS vulkan-capture approach) for fullscreen Vulkan games. *Capture pipeline done: `rivulet-core/src/vulkan_hook.rs` — 12 tests pass (Entry → Instance → Device → Queue, GPU selection, swapchain lifecycle, staging buffer readback via `capture_current_frame()`: image transition UNDEFINED → TRANSFER_SRC, `cmd_copy_image_to_buffer` → HOST_VISIBLE staging, map → RGBA pixels). Uses `ash` (0.38) for raw Vulkan API access. Next: VK_LAYER integration + `vkQueuePresentKHR` interception, then wire into recording pipeline. DoD: Vulkan fullscreen captured within the overhead budget, tests + docs.*
...
| Game capture (D3D/Vulkan/OpenGL hooks) | In progress (G2 DXGI done; G3 Vulkan capture pipeline + 12 tests; G4–G6 open) |
