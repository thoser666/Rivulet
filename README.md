...
- [x] Code signing (signing automation present, secrets needed — **waiting on certificate purchase**, see [#50](https://github.com/thoser666/Rivulet/issues/50))
...
- [x] **S5b – Browser source** — URL, interaction, transparency, config UI. *DoD: portable browser-source contract, bounded input queue, RGBA frame hand-off, configuration panel, tests + docs.* Done: `rivulet-core/src/browser_source.rs` validates `http(s)`/`file` URLs, manages viewport/FPS/zoom/transparency/CSS, queues interaction events, validates rendered RGBA frames, and exposes a platform-neutral adapter trait for WebView2/WebKitGTK/WKWebView. The Scenes view provides the configuration panel; native webview adapter wiring landed in PR [#227](https://github.com/thoser666/Rivulet/pull/227) (Windows wry/WebView2 adapter with real input forwarding + non-blocking frame poll + GUI backend tick).
...
- [x] **S8 – Audio sources** — per-app audio capture (Windows WASAPI loopback, Linux PipeWire) and audio input/output device capture as scene sources on Windows **and** Linux. *DoD: per-app and device audio selectable per scene, tests + docs. Tracked in [#68](https://github.com/thoser666/Rivulet/issues/68).* Done: `rivulet-core/src/audio_source.rs` (AudioSourceKind: Application/Input/Output/Mixed, AudioSource with volume/mute, 16 unit tests + i18n EN/DE). Follow-up ([#229](https://github.com/thoser666/Rivulet/issues/229)): the device half is live on Windows — `wasapi-out:<endpoint>`/`wasapi-in:<endpoint>` device ids (`DeviceTarget`), `rivulet-audio/src/device_capture.rs` (`list_audio_devices` + `AudioDeviceCapture`: shared-mode render loopback for output endpoints, plain capture for inputs, 48 kHz stereo f32), and a Windows Mixer device picker with friendly names + default marker so virtual mixer channels (SteelSeries GG/Sonar) are selectable as sources. Follow-up ([#231](https://github.com/thoser666/Rivulet/issues/231), Linux + macOS slices): the device picker + capture lifecycle are un-gated to Linux and macOS — Linux: `pw-src:<node>`/`pw-mon:<node>` device ids, `rivulet-audio/src/device_capture_pw.rs` (PipeWire registry enumeration of source nodes and sink monitors with `default`-metadata default markers; capture per `target.object` with `stream.capture.sink = true` for monitors, 48 kHz stereo f32); macOS: `core-audio-in:<name>` device ids, `rivulet-audio/src/device_capture_macos.rs` (cpal input enumeration with loopback-capable virtual drivers first, 48 kHz stereo f32 via worker-thread conversion; honest `OutputDevice` semantics — a render loopback needs a BlackHole-class driver, the picker shows the install hint). Same Mixer picker + live refresh on every platform.
...
- [x] **VOD track** — separate copyright-safe audio track (`VodTrack` on `StreamSettings`) for the Twitch VOD workflow while streaming: deterministic `enabled`/`recorded` model that is only ever active when both are set, so it can never silently leak into the live ingest; unit tests confirm the `ivod` flag signal and off-by-default behavior. In dual-output sessions an active VodTrack now emits a third, independent audio branch into the recording muxer (`audio_src_vod`, issue #78) — the live FLV path stays untouched. The routing UI is shipped and the streaming FLV mux stage carries the VOD-track marker (`metadatacreator=Rivulet-ivod` in the onMetaData; stock GStreamer cannot write a literal `ivod` AMF key — see the quality-gate design note).
...
**Status: Complete** (milestone closed; the remaining unchecked markers below are honest descopes and external waits, not engineering debt — see each annotation)

- [x] **Plugin system (WASM-first)** — the legacy "native Rust plugins" framing is retired: a native Rust plugin would just be a build of Rivulet itself, so the roadmap targets the WASM plugin system instead (manifest + sandboxed runtime + install/permission flow, shipped below); native-code extensibility remains available through the *temporary* OBS compatibility bridge (descoped to M11, see below)
- [x] **WASM plugin runtime core** (stable, sandboxed: plugins can never crash the app; long-term target plugin model) — *RFC Phases 1–3 shipped*: manifest parser/validator (`plugin_manifest.rs`), the wasmtime-based runtime (`plugin_runtime.rs`) with per-call fuel budgets, epoch-based wall-clock timeouts, the full Loaded → Initialized → Active → Inactive → Unloaded lifecycle and the core host imports, **plus the install/permission flow (Phase 3)**: Settings → Plugins scans the install root, identifies publisher/type/host-API/sandbox status per bundle before activation, records per-capability user approvals (`plugin_registry.rs`, default-denial, persisted), and gates enabling on a completed review — secrets/capture stay denied for WASM — see [`docs/plugin-system-rfc.md`](docs/plugin-system-rfc.md)
- **OBS plugin compatibility layer** — *moved to M11* ([#147](https://github.com/thoser666/Rivulet/issues/147); descoped at M5 closure): the temporary native bridge (opt-in "Compatibility Mode", explicitly marked unsafe, loads libobs encoders/filters/sources without UI; Qt UI plugins out of scope) is a transition convenience for existing OBS users, not a requirement for Rivulet's own plugin ecosystem — with RFC Phases 1–3 shipped, the WASM plugin system is the real, user-facing model and the bridge belongs with the M11 plugin-platform work
- [x] **obs-websocket compatibility** — OBS WebSocket v5 server (JSON) for the Stream Deck / TouchPortal ecosystem remote control with the exact v5 wire names (`StartRecord`/`ToggleRecord`, `StartStream`/`ToggleStream`, `ToggleInputMute`, pause/unpause, replay buffer start/stop/toggle/save, studio mode): scene list/switch, source listing, recording/streaming/pause/mute/replay/studio control, optional password authentication and event subscriptions, verified end-to-end with a real WebSocket client plus a Stream Deck compatibility contract test (see [`docs/obs-websocket.md`](docs/obs-websocket.md)). [#72](https://github.com/thoser666/Rivulet/issues/72)
- [x] **Windows/macOS feature parity** — window capture on Windows exists; **macOS recording implemented** (xcap monitor/window video capture + cpal audio: microphone out of the box, system audio via a loopback driver like BlackHole; Record view with monitor/window/preset selection, start/stop, finalizing status; see [`docs/macos-recording.md`](docs/macos-recording.md)); audio filters/monitoring on macOS remains a follow-up, and **live on-device verification is hardware-blocked** — the implementation is compile-checked in CI, but executing a real capture session needs access to a physical Mac (one manual session, not engineering work); see the scope note in [`docs/macos-recording.md`](docs/macos-recording.md)
...
- [x] Code signing (signing automation present, secrets needed — **waiting on certificate purchase**, see [#50](https://github.com/thoser666/Rivulet/issues/50): SignPath Foundation declined the free tier (insufficient stars), so Windows needs an OV certificate (~$100–200/yr) and macOS needs Apple Developer ($99/yr); Linux GPG signing is already active)
- [x] **Telemetry (opt-in, privacy-friendly)** — privacy-first opt-in telemetry pipeline (Settings → Telemetry, off by default): identifier-free event model (enums/codes/booleans only — never titles, paths, URLs, stream keys or usernames), opting out clears pending events, bounded deterministic collector (`rivulet-core::TelemetryReporter`), and a `TelemetrySink` interface with **no transport wired in the shipped build** — nothing leaves the device; events are wired end-to-end (`Startup`, `RecordingStop`, `RecordingError` with classified category, `SceneSwitch`, `ChatConnect`); the client-side contract and policy are shipped and test-locked, an HTTPS ingestion backend remains a documented follow-up (see [`docs/telemetry.md`](docs/telemetry.md)) *[#127](https://github.com/thoser666/Rivulet/issues/127)*
...
- [ ] **Multi-channel distribution rollout** — tracked in [`docs/release-platforms.md`](docs/release-platforms.md); the rollout continues past the milestone (Stage 1 live, Stage 2 prepared and waiting on external package reviews — see the stage list below), it does not block M5:
...
**Status: Complete** (all 4 features shipped and issues closed — chat auto-clips, multi-platform restream, mobile/HTTP remote companion, and multi-track audio routing; completion report in [`docs/m6-creator-toolkit-completion-report.md`](docs/m6-creator-toolkit-completion-report.md))
...
**Status: Complete** (all 4 issues closed — per-platform stream metadata, device picker, native scene compositing, and the browser-source contract + Windows WebView2 POC; the interim backlog milestone [#13](https://github.com/thoser666/Rivulet/milestone/13) collected post-M6 follow-up work that belongs in neither M7 (Automation & Determinism) nor M8 (Embeddable Engine & API))
...
- [x] **Native Browser-source engine** ([#215](https://github.com/thoser666/Rivulet/issues/215)) — shipped in PRs [#222](https://github.com/thoser666/Rivulet/pull/222), [#223](https://github.com/thoser666/Rivulet/pull/223) and [#227](https://github.com/thoser666/Rivulet/pull/227): the `BrowserSourceBackend` contract has a deterministic, headless reference implementation (`SyntheticBrowserBackend`) plus a `prime_browser_backend`/`sync_browser_backend` driver so the GUI↔backend wiring and the scene snapshot pipeline are fully testable, and the `rivulet-browser` crate ships a Windows adapter that embeds a real wry/WebView2 viewport captured via `ICoreWebView2::CapturePreview`, forwards pointer/keyboard input as Win32 message sequences (unit-tested mapping; no-op while interaction is disabled), toggles transparency via `ICoreWebView2Controller2`, and is driven from the Scenes view through a diff-based GUI tick (settings sync, input drain, non-blocking frame poll, egui preview texture); the `#[ignore]` spike test asserts a colour-pixel capture (GDI `PrintWindow`/`BitBlt` documented as a dead end); Linux/macOS adapters remain a follow-up (see `docs/browser-source-spike.md`)
- [x] **Device/window/monitor picker in scene items** ([#216](https://github.com/thoser666/Rivulet/issues/216)) — the scene add dialog shows a device picker for capture kinds (webcam/game/screen) backed by the same device tables as the record view; the chosen device is persisted on the source as `device_id` (`camera:…`/`game:…`/`monitor:…`)
- [x] **Native scene compositing** ([#217](https://github.com/thoser666/Rivulet/issues/217)) — `SceneSnapshot` now supports real RGBA `SnapshotFrame`s per layer via `attach_frames`; the compositor rasterizes cropped, scaled, rotated, opacity-blended source pixels (`render_rgba`) instead of only color tiles, and the scene snapshot export feeds browser frames straight in (F-M2-002 follow-up)

**Goal:** ✅ Closed the two visible gaps that keep Rivulet from feeling "production-ready" to OBS users — per-platform stream metadata (✅ #214) and real scene composition (✅ #216–#217).
...
| Browser sources | S5b contract/configuration done + Windows adapter (#227); Linux/macOS follow-up |
...
| VOD track | Done (config model + dual-output recording branch `audio_src_vod` + routing UI + streaming mux-stage marker, issue #78) |
...
| Chat dock & alerts | Partial (M5/M6: combined multi-platform chat dock — one list, one send box, all connected platforms (Twitch IRC + Kick WebSocket + YouTube polling) with per-line platform badges, broadcast send with per-platform delivery feedback, and platform-routed replies, see docs/twitch-chat.md; follow/sub/donation/raid ingestion surfaced locally in the chat dock **and** in a dedicated combined alerts dock — one live list for all platforms with platform badges, bounded independently and clearable, fed by an opt-in loopback webhook receiver for Streamlabs + Twitch EventSub with HMAC verification, and a native outbound Twitch EventSub WebSocket transport (wss://, forwarder-free, no port) — see docs/alerts-ingest.md) *[#129](https://github.com/thoser666/Rivulet/issues/129)* |
