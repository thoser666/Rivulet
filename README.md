...
| M2 – Scenes & Composition | Scenes, Sources, Game Capture, Scene Organisation, Transitions, Studio Mode | 🚧 In progress (scene management + G2 DXGI done; G3–G6 hooks open — G3 WIP draft not buildable, S1–S8 sources open) | — | [![M2](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.github.com%2Frepos%2Fthoser666%2FRivulet%2Fmilestones%2F1&query=open_issues&label=M2&color=blue)](https://api.github.com/repos/thoser666/Rivulet/milestones/1) |
...
- [ ] **G3 – Vulkan hook** — VK_LAYER-based capture (OBS vulkan-capture approach) for fullscreen Vulkan games. *WIP draft in `rivulet-core/src/vulkan_hook.rs` (reference only — does not compile yet, needs a rewrite against vulkano) and deliberately not wired into the build; tracked in [#56](https://github.com/thoser666/Rivulet/issues/56). DoD: Vulkan fullscreen captured within the overhead budget, tests + docs.*
