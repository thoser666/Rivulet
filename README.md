...
- [x] **Game capture live preview** — show a live thumbnail of the selected game window in the GUI before recording starts, so the user can verify the correct window is targeted. *Done: `GamePreview` grabs a frame via xcap on window selection and refreshes every 500ms, displayed as an `egui::Texture` below the game-window picker (Linux + Windows). Also fixed the Source dropdown leaking the selected window title.*
