...
- [x] **Windows/macOS feature parity** — window capture on Windows exists; **macOS screen/window recording implemented** (xcap monitor/window capture, video-only, Record view with monitor/window/preset selection, start/stop, finalizing status; see [`docs/macos-recording.md`](docs/macos-recording.md)); system/mic audio capture on macOS and live on-device verification remain follow-ups — as a release blocker, not an afterthought
