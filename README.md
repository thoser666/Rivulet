...
- [x] **Native Browser-source engine** ([#215](https://github.com/thoser666/Rivulet/issues/215)) — shipped in PRs [#222](https://github.com/thoser666/Rivulet/pull/222) and [#223](https://github.com/thoser666/Rivulet/pull/223): the `BrowserSourceBackend` contract has a deterministic, headless reference implementation (`SyntheticBrowserBackend`) plus a `prime_browser_backend` driver so the GUI↔backend wiring and the scene snapshot pipeline are fully testable, and the `rivulet-browser` crate proves the native path on Windows with a real wry/WebView2 viewport captured via `ICoreWebView2::CapturePreview` (`#[ignore]` spike test asserts a colour-pixel capture; GDI `PrintWindow`/`BitBlt` documented as a dead end); wiring a production adapter into the scene pipeline remains a follow-up (see `docs/browser-source-spike.md`)
