...
- [ ] **Multi-track audio routing** — capture individual app audio streams (game, Spotify, Discord) as separate named sources with independent filters and volume; route each source independently to Record and/or Stream outputs via a checkbox matrix; persists across restarts with i18n (EN+DE); per-platform capture: WASAPI per-app (Windows), PipeWire/PulseAudio per-app (Linux), system loopback fallback (macOS) — **engine core implemented (Phase 1, issue [#154](https://github.com/thoser666/Rivulet/issues/154))**: source/routing/filter types, per-source engine API, versioned `audio_routing_v1` persistence, and routing-aware pipelines for record (per-source tracks), stream (mixed FLV track), and dual output; GUI mixer and platform capture backends remain; see [`docs/m6-audio-routing.md`](docs/m6-audio-routing.md)
