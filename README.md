...
- [~] **Native Browser-source engine** ([#215](https://github.com/thoser666/Rivulet/issues/215)) — the `BrowserSourceBackend` contract now has a deterministic, headless reference implementation (`SyntheticBrowserBackend`) plus a `prime_browser_backend` driver, so the GUI↔backend wiring and the scene snapshot pipeline are fully testable without a native webview; embedding a real wry/WebView2/WebKitGTK/WKWebView adapter remains the final native step (see `docs/browser-source-spike.md`)
