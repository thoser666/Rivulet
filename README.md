...
- [ ] **G3 – Vulkan hook** — VK_LAYER-based capture (OBS vulkan-capture approach) for fullscreen Vulkan games. *Infrastructure done: `rivulet-core/src/vulkan_hook.rs` compiles and passes 8 tests (Entry → Instance → Device → Queue pipeline, GPU selection, device name, API version, swapchain lifecycle). Uses `ash` (v0.38) for raw Vulkan API access. Next: VK_LAYER integration + `vkQueuePresentKHR` interception for swapchain image capture, then wire into recording pipeline. DoD: Vulkan fullscreen captured within the overhead budget, tests + docs.*
...
| Game capture (D3D/Vulkan/OpenGL hooks) | In progress (G2 DXGI done; G3 Vulkan infra + 8 tests; G4–G6 open) |
