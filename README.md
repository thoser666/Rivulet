...
- [ ] **Game capture live preview** — show a live thumbnail of the selected game window in the GUI before recording starts, so the user can verify the correct window is targeted. *Implementation: grab a single frame via `windows-capture` or `xcap` on window selection, display as `egui::Texture` (same pattern as `RegionPreview`). Refresh on selection change or every ~500ms.*
