...
## [Unreleased]
- ci(release): trigger alpha releases from `build:`/`chore:` commits too — the release gate now treats `feat`/`fix`/`build`/`chore` as releasable (build/chore bump the patch version via `scripts/release-version.sh`), so packaging and CI-housekeeping changes ship automatically instead of needing a manual workflow dispatch (the alpha.98/alpha.99 gap); docs/test/style-only pushes still skip the release; ci_pinning guard + CONTRIBUTING.md updated
