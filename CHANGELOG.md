...
## [Unreleased]
- ci(wiki): audit wiki links into the repo docs — new `scripts/audit-wiki-links.py` validates target files and reproduces GitHub's heading-anchor slugs (plain HTTP checks always return 200 and miss anchor drift); wired into the scheduled wiki workflow (`--develop-from-origin`) and the local sync smoke as check 4; ci_pinning guard added; full audit ran clean (17 links across 21 pages)
