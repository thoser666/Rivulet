...
- fix(gui): restore persisted app state on startup — save() wrote the eframe storage but nothing read it back, silently dropping every configured setting (theme, Discord client id, hotkeys, OBS WebSocket, MIDI presets, …); RivuletApp::new now restores via eframe::get_value under APP_KEY and re-attaches the live engine + CLI timeout; eframe-storage round-trip tests and a ci_pinning guard added
